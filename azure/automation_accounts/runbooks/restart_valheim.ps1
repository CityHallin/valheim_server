...
<#
    .SYNOPSIS
        Restart Valheim Scheduled Task

    .DESCRIPTION
        This PowerShell script is meant to be run from an Azure Automation Account
        to restart a Valheim VM's Valheim scheduled task. 
...
Invoke-AzVMRunCommand -ResourceGroupName "Valheim-dev" -VMName "Valheim-dev" -CommandId 'RunPowerShellScript' -ScriptString "Start-ScheduledTask -TaskName Valheim_Start"
